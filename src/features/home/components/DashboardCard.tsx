...
import { ShoppingCart, BarChart3, FileText } from 'lucide-react'
  ...
  return (
    ...
    >
      <div className={`absolute top-0 right-0 w-40 h-40 rounded-full -mr-20 -mt-20 group-hover:scale-150 transition-transform duration-500 ${
        isOrange ? 'bg-orange-500/10' : isGreen ? 'bg-emerald-500/10' : 'bg-blue-500/10'
      }`} />
      <div className="relative p-8 space-y-4">
        <div className={`inline-flex p-4 rounded-2xl ${
          darkMode 
            ? isOrange ? 'bg-orange-500/20' : isGreen ? 'bg-emerald-500/20' : 'bg-blue-500/20'
            : isOrange ? 'bg-orange-100' : isGreen ? 'bg-emerald-100' : 'bg-blue-100'
        }`}>
          <Icon className={`w-8 h-8 ${
            darkMode
              ? isOrange ? 'text-orange-400' : isGreen ? 'text-emerald-400' : 'text-blue-400'
              : isOrange ? 'text-orange-600' : isGreen ? 'text-emerald-600' : 'text-blue-600'
          }`} />
        </div>
        <div>
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-2 ${
        darkMode
          ? isOrange ? 'bg-orange-500/50' : isGreen ? 'bg-emerald-500/50' : 'bg-blue-500/50'
          : isOrange ? 'bg-orange-500' : isGreen ? 'bg-emerald-500' : 'bg-blue-500'
      }`} />
    </Link>
  )
}
